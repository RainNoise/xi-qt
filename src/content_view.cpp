...
void ContentView::resizeEvent(QResizeEvent *event) {
	...
	auto visibleLines = (int)(size.height() / linespace) + 2;
...
VisibleLines ContentView::getFirstLastVisibleLines(const QRect& bound) {
	...
	auto firstVisible = std::max(0, (int)(std::ceilf((bound.y() - topPad + m_scrollOrigin.y()) / linespace)));
	auto lastVisible = std::max(0, (int)(std::floorf((bound.y() + bound.height() - topPad + m_scrollOrigin.y()) / linespace)));

	VisibleLines vlines(firstVisible, lastVisible);
	return vlines;
}
...
void ContentView::paint(QPainter& renderer, const QRect& dirtyRect) {
	...
	//renderer.fillRect(rect(), Qt::blue);
	...
	auto firstVisible = std::max(0, (int)(std::ceilf((dirtyRect.y() - topPad + m_scrollOrigin.y()) / linespace)));
	auto lastVisible = std::max(0, (int)(std::floorf((dirtyRect.y() + dirtyRect.height() - topPad + m_scrollOrigin.y()) / linespace)));
...
qint64 ContentView::getLinesHeight() {
	auto linespace = m_dataSource->fontMetrics->height();
	auto topPad = linespace - m_dataSource->fontMetrics->ascent();
	auto lines = m_dataSource->lines->height();
	auto totalHeight = lines * linespace + topPad;
	return totalHeight;
}
...
int ContentView::getCharWidth() {
	auto charWidth = m_dataSource->fontMetrics->maxWidth();
	return charWidth;
}
...
int ContentView::getXOff() {
	auto xOff = std::ceilf(m_dataSource->gutterWidth + m_x0);
	return xOff;
}
...
int ContentView::checkLineVisible(int line) {
	...
	if (line < fl.first) return -1;
	else if (line > fl.last) return 1;
	else return 0;
}
...
qreal ContentView::getLineColumnWidth(int line, int column) {
	...
	if (lineVisible == 0) {
		...
		auto first = std::min(totalLines, fl.first);
		auto last = std::min(totalLines, fl.last);
		auto lines = lineCache->getLines(RangeI(first, last + 1)); // fix
...
int ContentView::checkLineColumnPosition(int line, int column) {
	...
	if (delta < 0) return -1;
	else if (delta > width()) return 1;
	else return 0;
}

QPair<qint64, qint64> ContentView::posToLineColumn(const QPoint& pos) {
	auto viewRect = rect();
	...
	auto firstVisible = std::max(0, (int)(std::ceilf((viewRect.y() - topPad + m_scrollOrigin.y()) / linespace)));
	auto lastVisible = std::max(0, (int)(std::floorf((viewRect.y() + viewRect.height() - topPad + m_scrollOrigin.y()) / linespace)));
	...
	auto font = m_dataSource->defaultFont;
	auto lineNum = qMax(qint64(0), qint64((pos.y() - topPad) / linespace + first));
	auto line = lines[lineNum - first];

	auto textline = std::make_shared<TextLine>(line->getText(), font);
	auto column = textline->xToIndex(pos.x());

	QPair<qint64, qint64> result(lineNum, column);
...
void ContentView::scrollY(int y) {
	...
	//auto first = std::max(0, (int)(value / linespace ));
...
void ContentView::keyPressEvent(QKeyEvent *ev) {
	...
	case Qt::Key_Return:
		...
		return;
	case Qt::Key_Home:
		if (none) moveToBeginningOfLine();
		else if (ctrl && shift) moveToBeginningOfDocumentAndModifySelection();
		else if (shift) moveToBeginningOfLineAndModifySelection();
		else if (ctrl) moveToBeginningOfDocument();
		return;
	case Qt::Key_End:
		if (none) moveToEndOfLine();
		else if (ctrl && shift) moveToEndOfDocumentAndModifySelection();
		else if (shift) moveToEndOfLineAndModifySelection();
		else if (ctrl) moveToEndOfDocument();
		return;
	case Qt::Key_PageDown:
		if (none) scrollPageDown();
		else if (shift) pageDownAndModifySelection();
		return;
	case Qt::Key_PageUp:
		if (none) scrollPageUp();
		else if (shift) pageUpAndModifySelection();
		return;
	case Qt::Key_Up:
		if (none) moveUp();
		else if (shift) moveUpAndModifySelection();
		return;
	case Qt::Key_Down:
		if (none) moveDown();
		else if (shift)	moveDownAndModifySelection();
		return;
	case Qt::Key_Left:
		if (none) moveLeft();
		else if (ctrl && shift) moveWordLeftAndModifySelection();
		else if (ctrl) moveWordLeft();
		else if (shift) moveLeftAndModifySelection();
		return;
	case Qt::Key_Right:
		if (none) moveRight();
		else if (ctrl && shift)	moveWordRightAndModifySelection();
		else if (ctrl) moveWordRight();
		else if (shift) moveRightAndModifySelection();
		return;
	case Qt::Key_Z:
		if (ctrl) {	undo(); return;	}
		break;
	...
	case Qt::Key_X:
		...
		break;
	case Qt::Key_V:
		if (ctrl) {	paste(); return; }
		break;
	case Qt::Key_A:
		if (ctrl) {	selectAll(); return; }
		break;
	default:
		break;
	};
...
void ContentView::mouseDoubleClickEvent(QMouseEvent *e) {
	...
	m_connection->sendClick(m_file->viewId(), line, column, 0, 2);
